...
class Backend {
 public:
  ...
  /// Shut down and release any memory allocated within the seastar context.
...
class Engine final {
 ...
 public:
  ...
  Engine() = default;
  Engine(const variables_map& cfg, Backend* backend, uint32_t iodepth);
  ~Engine();
  ...
  static options_description get_options_description();
};
                              ...
                              OptionalTimePoint timeout)
{
  timer<Clock> timer;
  if (timeout) {
    timer.set_callback([&min] { min = 0; });
    timer.arm(timeout.value());
  }

  // poll until the minimum number of events are available
  size_t count;
  do {

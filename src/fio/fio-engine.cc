...
Engine::Engine(const variables_map& cfg, Backend* backend, uint32_t iodepth)
  : backend(backend)
{
  if (&engine())
...
Engine::~Engine()
{
  ...
  // allow the backend to clean up
  schedule(make_task([this] { return backend->shutdown(); }));
...
options_description Engine::get_options_description()
{
